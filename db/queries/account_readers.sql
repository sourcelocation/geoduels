...
SELECT u.id::text,coalesce(u.email,ui.email,''),coalesce(ui.provider_name,''),coalesce(u.avatar_url,ui.avatar_url,''),coalesce(u.account_type='registered' AND u.nickname_claimed_at IS NULL,false),coalesce(nullif(u.display_name,''),ui.provider_name,u.id::text),u.account_type,coalesce(u.is_admin,false),coalesce(u.is_moderator,false),coalesce(u.banned_at IS NOT NULL AND (u.ban_expires_at IS NULL OR u.ban_expires_at>now()),false),coalesce(u.ban_reason,'') FROM users u LEFT JOIN LATERAL (SELECT email,provider_name,avatar_url FROM user_identities WHERE user_id=u.id AND provider IN ('discord','google') ORDER BY CASE provider WHEN 'discord' THEN 0 WHEN 'google' THEN 1 ELSE 2 END,created_at ASC LIMIT 1) ui ON true WHERE u.id=$1;
