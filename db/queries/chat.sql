...
VALUES (sqlc.arg(message_id), sqlc.arg(conversation_id), nullif(sqlc.arg(team_match_id), '')::uuid, sqlc.arg(sender_user_id), sqlc.arg(sender_display_name), sqlc.arg(kind)::gd_chat_kind, sqlc.arg(body), sqlc.arg(emote), sqlc.arg(audience)::gd_chat_audience, nullif(sqlc.arg(team_id), '')::gd_team_id, sqlc.arg(created_at)) ON CONFLICT (id) DO NOTHING;
