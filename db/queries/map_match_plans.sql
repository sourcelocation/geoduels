...
select round_index,lat,lng,coalesce(country,'') AS country,pano_id,heading,pitch,map_id::text AS map_id from match_round_plans where match_id=sqlc.arg(match_id) order by round_index;

-- name: MapDisplayName :one
select display_name from maps where id=$1;

-- name: MapIdentity :one
select m.id::text AS id,coalesce((select a.alias from map_aliases a where a.map_id=m.id order by a.created_at,a.alias limit 1),m.id::text) AS map_key from maps m where m.id::text=sqlc.arg(map_key) or exists(select 1 from map_aliases a where a.map_id=m.id and a.alias=sqlc.arg(map_key)) limit 1;
...
select coalesce(owner_user_id::text,'') AS owner_user_id,visibility,status,display_name,location_count from maps where id=sqlc.arg(map_id) and archived_at is null for share;
