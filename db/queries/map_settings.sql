...
INSERT INTO site_settings(key, value_json, updated_at) VALUES ($1, $2::jsonb, now()) ON CONFLICT (key) DO UPDATE SET value_json=excluded.value_json, updated_at=now();

-- name: SetMapOfficial :execrows
UPDATE maps SET official_at=coalesce(official_at, now()), official_by=$2::uuid, published_at=coalesce(published_at, now()), visibility='public', updated_at=now() WHERE id=$1::uuid AND archived_at IS NULL;
