...
select replay_zstd, coalesce(replay_codec, 0)::int, coalesce(replay_uncompressed_bytes, 0), replay_sha256, replay_json::text
...
case when h.mode = 'singleplayer' then 'completed' when h.winner_user_id is null then 'draw' when h.winner_user_id = p.user_id then 'win' else 'loss' end,
coalesce(h.ranked, false) and h.mode = 'duel', coalesce(p.final_ranked_delta, 0), coalesce(p.total_score, 0), coalesce(opponent.user_id, ''), coalesce(opponent.display_name, '')
...
where p.user_id = $1 and h.mode = 'duel' and coalesce(h.ranked, false) and (p.ended_at, p.match_id) < ($3, $4::uuid)
order by p.ended_at desc, p.match_id desc limit $2;

-- name: ListPlayerMatchHistoryBasic :many
select h.match_id, h.mode, h.started_at, h.ended_at, coalesce(h.winner_user_id::text, ''), case when h.mode = 'singleplayer' then 'completed' when h.winner_user_id is null then 'draw' when h.winner_user_id = p.user_id then 'win' else 'loss' end, coalesce(h.ranked, false) and h.mode = 'duel', coalesce(p.final_ranked_delta, 0), coalesce(p.total_score, 0), coalesce(opponent.user_id, ''), coalesce(opponent.display_name, '')
...
select h.match_id,h.mode,h.started_at,h.ended_at,coalesce(h.winner_user_id::text,''),case when h.mode='singleplayer' then 'completed' when h.winner_user_id is null then 'draw' when h.winner_user_id=p.user_id then 'win' else 'loss' end,coalesce(h.ranked,false) and h.mode='duel',coalesce(p.final_ranked_delta,0),coalesce(p.total_score,0),coalesce(opponent.user_id,''),coalesce(opponent.display_name,'') from match_players p join match_history h on h.match_id=p.match_id left join lateral (select op.user_id::text user_id,coalesce(nullif(op.display_name,''),nullif(u.display_name,''),op.user_id::text) display_name from match_players op left join users u on u.id=op.user_id where op.match_id=p.match_id and op.user_id<>p.user_id order by op.total_score desc,op.user_id limit 1) opponent on true where p.user_id=$1 and (p.ended_at,p.match_id)<($3,$4::uuid) order by p.ended_at desc,p.match_id desc limit $2;
