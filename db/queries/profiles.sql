...
with season_matches as (
  select h.ended_at,h.match_id,h.winner_user_id=$1::uuid won
  from match_players mp join match_history h on h.match_id=mp.match_id
  where mp.user_id=$1 and h.mode='duel' and h.ranked
    and ($2::timestamptz is null or h.ended_at >= $2)
), streak_groups as (
...
-- name: GetProfile :one
select coalesce(nullif(u.display_name, seed.user_id::text), ui.provider_name, $1::text) display_name, coalesce(u.avatar_url,ui.avatar_url,'') avatar_url, coalesce(r.mmr,$4::int) mmr, coalesce(r.rd,$5::int) rating_rd, greatest(coalesce(us.games_played,0),coalesce(history_stats.games_played,0)) games_played, greatest(coalesce(us.wins,0),coalesce(history_stats.wins,0)) wins, coalesce(rs.games_played,0) ranked_games_played, coalesce(rs.wins,0) ranked_wins, coalesce(u.account_type='guest',false) is_guest, coalesce(u.is_admin,false) is_admin, coalesce(u.is_moderator,false) is_moderator, coalesce(u.banned_at is not null and (u.ban_expires_at is null or u.ban_expires_at>now()),false) is_banned, coalesce(u.ban_reason,'') ban_reason, coalesce(u.selected_badge_code,0)::smallint selected_badge_code from (select $1::uuid user_id) seed left join users u on u.id=seed.user_id left join lateral (select provider_name,avatar_url from user_identities where user_id=seed.user_id and provider='google' order by created_at asc limit 1) ui on true left join ranks r on r.user_id=seed.user_id and r.mode=$2 and r.season_id=$3 left join user_stats us on us.user_id=seed.user_id left join lateral (select count(*)::int games_played,count(*) filter(where h.winner_user_id=seed.user_id)::int wins from match_players mp join match_history h on h.match_id=mp.match_id where mp.user_id=seed.user_id and h.mode='duel') history_stats on true left join ranked_stats rs on rs.user_id=seed.user_id and rs.mode=$2 and rs.season_id=$3;

-- name: GetPublicProfile :one
select u.id::text user_id, coalesce(nullif(u.display_name,''),ui.provider_name,u.id::text) display_name, coalesce(u.avatar_url,ui.avatar_url,'') avatar_url, coalesce(r.mmr,$4::int) mmr, coalesce(r.rd,$5::int) rating_rd, greatest(coalesce(us.games_played,0),coalesce(history_stats.games_played,0))::int games_played, greatest(coalesce(us.wins,0),coalesce(history_stats.wins,0))::int wins, coalesce(rs.games_played,0)::int ranked_games_played, coalesce(rs.wins,0)::int ranked_wins, coalesce(u.selected_badge_code,0)::smallint selected_badge_code from users u left join lateral (select provider_name,avatar_url from user_identities where user_id=u.id and provider='google' order by created_at asc limit 1) ui on true left join ranks r on r.user_id=u.id and r.mode=$2 and r.season_id=$3 left join user_stats us on us.user_id=u.id left join lateral (select count(*)::int games_played,count(*) filter(where h.winner_user_id=u.id)::int wins from match_players mp join match_history h on h.match_id=mp.match_id where mp.user_id=u.id and h.mode='duel') history_stats on true left join ranked_stats rs on rs.user_id=u.id and rs.mode=$2 and rs.season_id=$3 where u.account_type='registered' and u.nickname_claimed_at is not null and lower(u.display_name)=lower($1);
...
update users set selected_badge_code=nullif($2,0) where id=$1;
