...
update friend_requests set status='cancelled',responded_at=now() where status='pending' and least(sender_user_id,recipient_user_id)=least($1::uuid,$2::uuid) and greatest(sender_user_id,recipient_user_id)=greatest($1::uuid,$2::uuid);
...
insert into friendships(user_id_low,user_id_high,created_from_request_id) values(least($1::uuid,$2::uuid),greatest($1::uuid,$2::uuid),$3) on conflict do nothing;

-- name: InsertUserEvent :exec
insert into user_events(user_id,sequence,type,payload_json) values($1,$2,$3,$4::jsonb);
...
with recent as (select mp2.user_id,max(h.ended_at) shared_at from match_players mine join match_history h on h.match_id=mine.match_id join match_players mp2 on mp2.match_id=h.match_id and mp2.user_id<>$1 where mine.user_id=$1 and h.ended_at is not null group by mp2.user_id)
select u.id::text user_id,u.display_name,coalesce(u.avatar_url,'') avatar_url,coalesce(r.mmr,1000)::int mmr,u.last_seen_at,recent.shared_at::timestamptz shared_at from recent join users u on u.id=recent.user_id left join ranks r on r.user_id=u.id and r.mode='duel' where u.account_type='registered' and u.nickname_claimed_at is not null and u.social_discoverable and not exists(select 1 from friendships f where f.user_id_low=least($1::uuid,u.id) and f.user_id_high=greatest($1::uuid,u.id)) and not exists(select 1 from friend_requests fr where fr.status='pending' and least(fr.sender_user_id,fr.recipient_user_id)=least($1::uuid,u.id) and greatest(fr.sender_user_id,fr.recipient_user_id)=greatest($1::uuid,u.id)) and not exists(select 1 from user_blocks b where (b.blocker_user_id=$1 and b.blocked_user_id=u.id) or (b.blocker_user_id=u.id and b.blocked_user_id=$1)) order by recent.shared_at desc limit $2;
...
update user_notifications set read_at=coalesce(read_at,now()) where user_id=$1 and dedupe_key='friend_request:'||$2;

-- name: MarkPartyInvitationNotificationRead :exec
update user_notifications set read_at=coalesce(read_at,now()) where user_id=$1 and dedupe_key='party_invitation:'||$2;
...
select p.invite_code,p.mode,count(pm.user_id)::int member_count from parties p join party_members self on self.party_id=p.id and self.user_id=$2 and self.left_at is null left join party_members pm on pm.party_id=p.id and pm.left_at is null where p.id=$1 and p.state='open' and p.expires_at>now() and exists(select 1 from friendships f where f.user_id_low=least($2::uuid,$3::uuid) and f.user_id_high=greatest($2::uuid,$3::uuid)) and exists(select 1 from users u where u.id=$3 and u.social_party_invites_enabled) and not exists(select 1 from user_blocks b where (b.blocker_user_id=$2 and b.blocked_user_id=$3) or (b.blocker_user_id=$3 and b.blocked_user_id=$2)) group by p.id;
